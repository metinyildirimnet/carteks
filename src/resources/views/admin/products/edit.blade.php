...
<div class="row">
    <div class="col-md-12">
        <div class="card card-primary">
            ...
            <!-- /.card-header -->
            <!-- form start -->
            <form id="product-form" action="{{ route('admin.products.update', $product->slug) }}" method="POST">
                @csrf
                @method('PUT')
                <div class="card-body">
                    ...
                    <div class="form-group">
                        ...
                        <input type="number" id="discounted_price" name="discounted_price" class="form-control" step="0.01" value="{{ old('discounted_price', $product->discounted_price) }}">
                    </div>
                    ...
                    <div class="form-group">
                        <div class="custom-control custom-switch">
                            <input type="checkbox" class="custom-control-input" id="is_package" name="is_package" {{ $product->is_package ? 'checked' : '' }}>
                            ...
                            @foreach($usageAreas as $area)
                                <div class="col-md-3 col-sm-6">
                                    <div class="custom-control custom-checkbox">
                                        <input class="custom-control-input" type="checkbox" id="usage_area_{{ Str::slug($area) }}" name="usage_areas[]" value="{{ $area }}" {{ in_array($area, old('usage_areas', $product->usage_areas ?? [])) ? 'checked' : '' }}>
                    ...
                    <div id="package-products-section" style="{{ $product->is_package ? '' : 'display: none;' }}">
                        <div class="form-group">
                        ...
                        <table id="package-items-table" class="table table-bordered mt-2">
                            ...
                            <tbody>
                                @if($product->is_package)
                                    ...
                                    <tr data-id="{{ $containedProduct->id }}">
                                        <td>
                                            {{ $containedProduct->title }}
                                            <input type="hidden" name="contained_products[][id]" value="{{ $containedProduct->id }}">
                                        </td>
                                        <td>
                                            <input type="number" name="contained_products[][quantity]" class="form-control" value="{{ $containedProduct->pivot->quantity }}" min="1">
                                        </td>
                                        <td>
                    ...
                    <div class="form-group">
                        <label for="recommended_products">Önerilen Ürünler</label>
                        <select id="recommended_products" name="recommended_products[]" class="form-control" multiple>
                            @foreach ($products as $p)
                                <option value="{{ $p->id }}" {{ in_array($p->id, $recommendedProductIds) ? 'selected' : '' }}>
                                    {{ $p->title }}
                                </option>
                            @endforeach
                        </select>
                        ...
                        <div style="display: flex; flex-wrap: wrap; gap: 10px;">
                            @forelse ($product->images as $image)
                                <div style="position: relative;">
                                    <img src="{{ $image->image_path }}" alt="{{ $product->title }}" class="img-thumbnail" style="width: 100px; height: 100px; object-fit: cover;">
                                </div>
                            @empty
                                <p>Mevcut görsel bulunmuyor.</p>
                            @endforelse
                        </div>
                    </div>

                    <div class="form-group">
                        <label for="images">Yeni Ürün Görselleri Ekle</label>
...
document.addEventListener('DOMContentLoaded', function() {
    // Initialize Select2
    $('#recommended_products').select2({ theme: 'bootstrap4' });
    $('#product-selector').select2({ theme: 'bootstrap4' });
    ...
    function updateProductSelectorOptions() {
        ...
        packageItemsTbody.querySelectorAll('tr').forEach(row => {
            existingIds.add(row.dataset.id);
        });

        productSelector.querySelectorAll('option').forEach(option => {
    ...
    form.addEventListener('submit', async function(e) {
        ...
        // Delete the original, non-indexed fields from formData
        formData.delete('contained_products[][id]');
        formData.delete('contained_products[][quantity]');

        // Manually structure and append contained_products data
